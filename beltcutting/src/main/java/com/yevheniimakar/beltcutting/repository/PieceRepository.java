...
public interface PieceRepository extends JpaRepository<Piece, Long> {
    ...
    @Query("select p from Piece p  join fetch Card c where c.id = :id and p.size <> 0 order by p.piecesNumber")
    List<Piece> findByCardId(Long id);

    @Query("select p.piecesNumber from Piece p join fetch Card c where c = :card order by p.piecesNumber")
